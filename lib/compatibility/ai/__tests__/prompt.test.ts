...
const mockBreakdown = {
  saju: {
    score: 68,
    reason: '무난한 관계입니다',
    details: ['화(火)이 목(木)를 생함 - 상생(相生) 관계'],
  },
  zodiac: {
    score: 75,
    reason: '불 원소 궁합',
    details: ['양자리-사자자리 불 원소 상생'],
  },
  mbti: {
    score: 90,
    reason: 'INTJ+ENFP 황금 쌍',
    details: ['인지기능 상호보완'],
  },
}
...
describe('buildCompatibilityPrompt', () => {
  ...
  it('연인 관계는 친밀도 섹션이 포함되어 비연인보다 길다', () => {
    const loverPrompt = buildCompatibilityPrompt({
      person1,
      person2,
      relationshipType: 'lover',
      totalScore: 80,
    ...
    const colleaguePrompt = buildCompatibilityPrompt({
      person1,
      person2,
